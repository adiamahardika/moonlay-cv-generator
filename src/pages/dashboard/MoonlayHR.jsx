...
export default function MoonlayHR() {
  ...
  const [selectedRows, setSelectedRows] = useState(null); // Selected rows
  const [rowClick, setRowClick] = useState(true);
  const [pdfUrl, setPdfUrl] = useState(null); // Updated type
  const [docxUrl, setDocxUrl] = useState(null); // Updated type
  const [showDialog, setShowDialog] = useState(false); // Updated state for PrimeReact Dialog
  const [embedPdfUrl, setembedPdfUrl] = useState(null); // Updated type
  ...
  const confirmDelete = () => {
    const idToDelete = selectedRows.map((row) => row.applicant_id);
  ...
  const exportCSV = () => {
    ...
    const csvContent = [
      ...
      ...selectedRows.map((row) => cols.map((col) => row[col.field]).join(',')), // Data rows
    ].join('\n');
...
const exportPdf = () => {
  ...
  import('jspdf').then((jsPDF) => {
    import('jspdf-autotable').then(() => {
      ...
      // Utility function to clean HTML tags and replace <br> with spaces
      ...
      // Map the selected rows to fit the exportColumns structure, cleaning text
      const data = selectedRows.map((row) => {
        return {
          applicant_dateofapplication: cleanText(row.applicant_dateofapplication),
          applicant_name: cleanText(row.applicant_name),
          applicant_contact: cleanText(row.applicant_contact),
          applicant_email: cleanText(row.applicant_email),
          applicant_education: cleanText(row.applicant_education),
          applicant_skill: cleanText(row.applicant_skill),
          applicant_experience: cleanText(row.applicant_experience),
        };
      });
      ...
      doc.autoTable({
        ...
        body: data, // Use cleaned selectedRows for the table data
